...
import { ArrowDownIcon, Pencil, PlusIcon, Trash2 } from 'lucide-react';
...
export function ExpenseList({ expenses }: { expenses: Expense[] }) {
    ...
    const filteredTransactions = expenses.filter((transaction) => {
        ...
        if (filter === 'discrecional') return matchesSearch && transaction.type === 'discrecional';
    ...
    const confirmDelete = () => {
        if (currentTransactionId) {
            ...
            console.log('Transacción eliminada:', currentTransactionId);

            router.delete(route('expense.destroy', currentTransactionId), {
                onSuccess: () => {
    ...
    const saveEditedTransaction = (e: React.FormEvent) => {
        ...
        if (editTransaction) {
            router.put(
                route('expense.update', editTransaction.id),
                {
                    ...editTransaction,
                },
                {
                                ...
                                filteredTransactions.map((transaction) => (
                                    <TableRow key={transaction.id}>
                                        ...
                                        <TableCell className="text-center">
                                            <span
                                                className={`flex items-center justify-center font-medium ${transaction.type === 'ingreso' ? 'text-red-600' : 'text-red-600'}`}
                                            >
